...
namespace Tibia.Objects
{
    ...
    public class Inventory
    {
        ...
        public int CountItems(uint id)
        {
            int count = 0;
            
            foreach (Item item in GetItems().Where(i => i.Id == id))
            {
                count += item.Count;
            }
        ...
        {
            List<Item> items = GetItems().Where(i => i.GetFlag(Tibia.Addresses.DatItem.Flag.IsStackable) && i.Count < 100).ToList();
